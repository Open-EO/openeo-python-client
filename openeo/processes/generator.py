...
class PythonRenderer:
    ...
    def render_process(self, process: Process, prefix: str = "", width: int = DEFAULT_WIDTH) -> str:

        # TODO: add type hints
        # TODO: width limit?
        def_line = "def {id}({args}):".format(
...
def generate_process_py(processes_dir: Union[Path, str], output=sys.stdout):
    ...
    """)
    fun_renderer = PythonRenderer(body_template="return process({id!r}, {args})", optional_default="UNSET")
    oo_renderer = PythonRenderer(oo_mode=True, body_template="return {id}({args})", optional_default="UNSET")
    for p in processes:
