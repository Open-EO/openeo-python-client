...
def test_band_basic(connection, api_version, test_data):
...
def test_indexing_100(con100, test_data):
    ...
    expected_graph = test_data.load_json("1.0.0/band_red.json")
    ...
    expected_graph = test_data.load_json("1.0.0/band_red_filtered.json")
...
def test_ndvi_reduce_bands_udf(connection, api_version, test_data):
    ...
    expected_graph = test_data.load_json("%s/udf_graph.json" % api_version)["process_graph"]
    assert actual_graph == expected_graph


def test_ndvi_reduce_bands_udf_legacy_v100(con100, test_data):
    # TODO #181 #312 drop this deprecated pattern
    s2_radio = con100.load_collection("SENTINEL2_RADIOMETRY_10M")
    ndvi_coverage = s2_radio.reduce_bands_udf("def myfunction(tile):\n    print(tile)\n    return tile")
    actual_graph = get_download_graph(ndvi_coverage)
    expected_graph = test_data.load_json("1.0.0/udf_graph.json")["process_graph"]
    assert actual_graph == expected_graph


def test_ndvi_reduce_bands_udf_v100(con100, test_data):
    s2_radio = con100.load_collection("SENTINEL2_RADIOMETRY_10M")
...
def test_invert_band(connection, api_version, test_data):
    cube = connection.load_collection("S2")
    band = cube.band("B04")
    result = ~band
    assert result.flat_graph() == test_data.load_json("%s/bm_invert_band.json" % api_version)


def test_eq_scalar(connection, api_version, test_data):
    cube = connection.load_collection("S2")
    band = cube.band("B04")
    result = band == 42
    assert result.flat_graph() == test_data.load_json("%s/bm_eq_scalar.json" % api_version)


def test_gt_scalar(connection, api_version, test_data):
    cube = connection.load_collection("S2")
    band = cube.band("B04")
    result = band > 42
...
def test_add_bands(connection, api_version, test_data):
    cube = connection.load_collection("S2")
    b4 = cube.band("B04")
    b3 = cube.band("B03")
    result = b4 + b3
    assert result.flat_graph() == test_data.load_json("%s/bm_add_bands.json" % api_version)
...
def test_logical_not_equal(connection, api_version, test_data):
    s2 = connection.load_collection("SENTINEL2_SCF")
    scf_band = s2.band("SCENECLASSIFICATION")
    mask = scf_band != 4
    actual = get_download_graph(mask)
    assert actual == test_data.load_json("%s/notequal.json" % api_version)


def test_logical_or(connection, api_version, test_data):
    s2 = connection.load_collection("SENTINEL2_SCF")
    scf_band = s2.band("SCENECLASSIFICATION")
    mask = (scf_band == 2) | (scf_band == 5)
    actual = get_download_graph(mask)
    assert actual == test_data.load_json("%s/logical_or.json" % api_version)


def test_logical_and(connection, api_version, test_data):
    s2 = connection.load_collection("SENTINEL2_SCF")
    b1 = s2.band("SCENECLASSIFICATION")
    b2 = s2.band("MSK")
    mask = (b1 == 2) & (b2 == 5)
    actual = get_download_graph(mask)
    assert actual == test_data.load_json("%s/logical_and.json" % api_version)


def test_merge_cubes_or(connection, api_version, test_data):
    ...
    assert actual == test_data.load_json("%s/merge_cubes_or.json" % api_version)


def test_merge_cubes_multiple(connection, api_version, test_data):
    s2 = connection.load_collection("S2")
    b1 = s2.band("B02")
    b1 = b1.linear_scale_range(0, 1, 0, 2)
    combined = b1 + b1 + b1
    ...
    assert actual == test_data.load_json("%s/merge_cubes_multiple.json" % api_version)


def test_fuzzy_mask(connection, api_version, test_data):
    s2 = connection.load_collection("SENTINEL2_SCF")
    scf_band = s2.band("SCENECLASSIFICATION")
    clouds = scf_band == 4
    fuzzy = clouds.apply_kernel(kernel=0.1 * np.ones((3, 3)))
    mask = fuzzy > 0.3
    assert mask.flat_graph() == test_data.load_json("%s/fuzzy_mask.json" % api_version)


def test_fuzzy_mask_band_math(con100, test_data):
    s2 = con100.load_collection("SENTINEL2_SCF")
    scf_band = s2.band("SCENECLASSIFICATION")
    clouds = scf_band == 4
    fuzzy = clouds.apply_kernel(kernel=0.1 * np.ones((3, 3)))
    mask = fuzzy.add_dimension("bands", "mask", "bands").band("mask") > 0.3
    assert mask.flat_graph() == test_data.load_json("1.0.0/fuzzy_mask_add_dim.json")


def test_normalized_difference(connection, api_version, test_data):
    cube = connection.load_collection("S2")
    nir = cube.band("B08")
    red = cube.band("B04")
    ...
    assert result.flat_graph() == test_data.load_json("%s/bm_nd_bands.json" % api_version)


def test_ln(con100, test_data):
    result = con100.load_collection("S2").band("B04").ln()
    assert result.flat_graph() == test_data.load_json("1.0.0/bm_ln.json")


def test_log10(con100, test_data):
    result = con100.load_collection("S2").band("B04").log10()
    assert result.flat_graph() == test_data.load_json("1.0.0/bm_log.json")
