...
def test_comparison(connection, api_version, operation, expected):
    ...
    assert result.flat_graph() == load_json_resource(
        "data/%s/bm_comparison.json" % api_version, preprocess=lambda data: data.replace("OPERATOR", expected)
    )
...
def test_log2(con100):
    ...
    assert result.flat_graph() == load_json_resource(
        "data/1.0.0/bm_log.json", preprocess=lambda s: s.replace('"base": 10', '"base": 2')
    )
...
def test_log3(con100):
    ...
    assert result.flat_graph() == load_json_resource(
        "data/1.0.0/bm_log.json", preprocess=lambda s: s.replace('"base": 10', '"base": 3')
    )
