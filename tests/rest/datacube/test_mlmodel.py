...
def test_load_ml_model_basic(con100, id):
    ...
    assert ml_model.flat_graph() == {
        "loadmlmodel1": {"process_id": "load_ml_model", "arguments": {"id": id}, "result": True}
    }
...
def test_load_ml_model_from_job(con100):
    ...
    assert isinstance(ml_model, MlModel)
    assert ml_model.flat_graph() == {
        "loadmlmodel1": {"process_id": "load_ml_model", "arguments": {"id": "my-j08"}, "result": True}
    }
                ...
                "reducer": {
                    "process_graph": {
                        "predictrandomforest1": {
                            ...
                            "arguments": {"data": {"from_parameter": "data"}, "model": {"from_parameter": "context"}},
                            "result": True,
                        }
                    }
                },
