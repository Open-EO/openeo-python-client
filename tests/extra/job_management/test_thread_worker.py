...
class TestJobManagerWorkerThreadPool:
    ...
    def test_worker_thread_lifecycle(self, worker, caplog):
        with caplog.at_level(logging.INFO):
            assert not worker._executor._shutdown
            worker.shutdown()
            assert worker._executor._shutdown
        assert "Shutting down worker thread pool" in caplog.text

    def test_process_futures_no_completed_futures(self, worker, stats, caplog):
        with caplog.at_level(logging.INFO):
            # Submit work without any backend response mock
            worker.submit_work(worker.WORK_TYPE_START_JOB, ("https://foo.test", "token", "job-123"))
    ...
    def test_start_job_success(self, worker, stats, requests_mock):
    ...
    def test_start_job_failure(self, worker, stats, requests_mock, caplog):
    ...
    def test_process_futures_mixed_success_and_failure(self, worker, stats, requests_mock, caplog):
        # Successful job
    ...
    def test_invalid_work_args(self, worker, stats, caplog, work_args, expected_log):
        with caplog.at_level(logging.ERROR):  
            with pytest.raises(Exception):  # Expect an exception to be raised
                worker.submit_work(worker.WORK_TYPE_START_JOB, work_args)
