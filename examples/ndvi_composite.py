...
composited_ndvi = masked.aggregate_temporal(
    intervals=[start_date, "2018-05-21", "2018-05-01", end_date],
    labels=["2018-05-01", "2018-05-11"],
    reducer=lambda d: d.mean(),
    dimension="t",
)
...
def test_composite_by_udf():
    masked.apply_dimension(process=compositing_udf, dimension="t").filter_temporal(start_date, end_date).download(
        "composite_udf.nc", format="NetCDF"
    )
