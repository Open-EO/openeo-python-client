...
l1c = connection.load_collection(
    ...
    bands=["B09", "B8A", "B11", "sunAzimuthAngles", "sunZenithAngles", "viewAzimuthMean", "viewZenithMean"],
)
...
def test_sentinel2_sen2cor():
    # create image collection
    rgb = connection.load_collection(
    ...
    # specify process graph
    download = rgb.download("/tmp/openeo-rgb-sen2cor.geotiff", format="GTiff")
    print(download)
...
polygon = Polygon(
    shell=[
        [5.152158737182616, 51.18469636040683],
        [5.15183687210083, 51.181979395425095],
        [5.152802467346191, 51.18192559252128],
        [5.153381824493408, 51.184588760878924],
        [5.152158737182616, 51.18469636040683],
    ]
)
...
def test_icor_timeseries():
    cloudmask = connection.load_collection(
        "SENTINEL2_L2A_SENTINELHUB", temporal_extent=["2019-02-01", "2019-11-01"], bands=["CLM"]
    )
    ...
    l2a = connection.load_collection(
        "SENTINEL2_L2A_SENTINELHUB", temporal_extent=["2019-02-01", "2019-11-01"], bands=["B08", "B04"]
    )
    start_time = time.time()
